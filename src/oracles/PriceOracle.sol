...
contract PriceOracle is ICPriceOracle {
        ...
        returns (uint160 sqrtPriceX96)
    {
        ...
        (int24 arithmeticMeanTick,) = OracleLibrary.consult(oracleData.pool, oracleData.secondsAgo);

        require(token0 == IUniswapV3Pool(oracleData.pool).token0(), "oracle: invalid token0");
        require(token1 == IUniswapV3Pool(oracleData.pool).token1(), "oracle: invalid token1");
