...
abstract contract CConstantProductTestHarness is BaseComposableCoWTest {
    ...
    uint160 DEFAULT_NEW_PRICE_OTHER_SIDE_X96 = CMathLib.getSqrtPriceFromPrice(5499 ether);
    ...
    function setUp() public virtual override(BaseComposableCoWTest) {
        ...
        constantProduct = new CConstantProduct(solutionSettler, IERC20(USDC), IERC20(WETH));
    ...
    function getDefaultTradingParams() internal view returns (CConstantProduct.TradingParams memory) {
    ...
    function setUpDefaultTradingParams() internal view returns (CConstantProduct.TradingParams memory) {
    ...
    function setUpDefaultReserves(address owner) internal {
        (, uint256 ownerReserve0, uint256 ownerReserve1) = calculateProvidedLiquidityDefault();
        setUpReserves(owner, ownerReserve0, ownerReserve1);
    }

    function setUpReserves(address owner, uint256 amount0, uint256 amount1) internal {
        vm.mockCall(address(constantProduct.token0()), abi.encodeCall(IERC20.balanceOf, (owner)), abi.encode(amount0));
        vm.mockCall(address(constantProduct.token1()), abi.encodeCall(IERC20.balanceOf, (owner)), abi.encode(amount1));
    }
    ...
    function setUpOracleResponse(uint160 newSqrtPriceX96, address oracle, address _token0, address _token1) public {
        vm.mockCall(
            oracle,
            abi.encodeCall(ICPriceOracle.getSqrtPriceX96, (_token0, _token1, DEFAULT_PRICE_ORACLE_DATA)),
    ...
    function setUpOracleResponse(
        ...
        vm.mockCall(
            oracle,
            abi.encodeCall(ICPriceOracle.getSqrtPriceX96, (_token0, _token1, priceOracleData)),
            abi.encode(newSqrtPriceX96)
        );
    }
    ...
    function checkedGetTradeableOrder(CConstantProduct.TradingParams memory tradingParams)
        internal
        view
    ...
    function calculateProvidedLiquidityDefault() internal view returns (uint128, uint256, uint256) {
        uint128 _liquidity = CMathLib.getLiquidityFromAmountsSqrtPriceX96(
            DEFAULT_PRICE_CURRENT_X96, DEFAULT_PRICE_UPPER_X96, DEFAULT_PRICE_LOWER_X96, 1 ether, 5000 ether
        );

        (uint256 _amount0, uint256 _amount1) = CMathLib.getAmountsFromLiquiditySqrtPriceX96(
            DEFAULT_PRICE_CURRENT_X96, DEFAULT_PRICE_UPPER_X96, DEFAULT_PRICE_LOWER_X96, _liquidity
        );
